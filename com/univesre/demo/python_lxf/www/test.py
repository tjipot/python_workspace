...
async def test(loop):
	await create_pool(loop = loop, host = 'localhost', port = 3306, user = 'www-data', password = 'www-data', database = 'univesre')
	# u = User(name = 'Test', email = 'test@example.com', passwd = '1234567890', image = 'about:blank')
	# yield from u.save()
	rows = await User.findNumber() # countRows()
	...
	if rows < 3:
		for idx in range(5):
			...
			row = await User.findNumber(where = 'email = ?', args = [u.email])
			if row == 0:
				await u.save()
			else:
